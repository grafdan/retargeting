...
@implementation RetargetingImage
@synthesize image = _image;
-(void)rotateImageToOrientation {
    
    // this code is based on the public domain implementation on
    ...
    //    printf("image rotation: %d and size %f %f\nscale ratio %f\n",orient, width, height,scaleRatio);
    self.image = imageCopy;

}
-(void)scaleImageToSize:(int)size {
    [self rotateImageToOrientation];
    ...
    CGFloat scaleRatio;
    if(width>height) {
        scaleRatio = 1.0 * size / width;
    } else {
        scaleRatio = 1.0 * size / height;
    }
    CGRect bounds = CGRectMake(0, 0, scaleRatio*width, scaleRatio*height);
    ...
    NSLog(@"resized to %lf %lf",bounds.size.width,bounds.size.height);
